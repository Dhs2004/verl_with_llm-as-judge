...
from util import calc_accuracy4math, calc_cloud_score
...
async def get_reward2(request: Request):
    ...
    def wrap_process_data():
        ...
        pred_answer = json_data.get("response_str", "")
        groud_truth = json_data.get("ground_truth", "")
        #print(f"data is (in json):{json_data}")
        print(f"📥 输入数据:")
        #print(f"   data_source: {dataset_source}")
        #print(f"   ground_truth: {groud_truth}")
        print(f"   response_str: {pred_answer}")  # 只打印前200字符
        ...
        if dataset_source.find("cloud") == -1:
            ...
            # 详细调试格式奖励
            def debug_format_reward(text: str) -> float:
                #print(f"🔍 检查格式奖励: {text[:100]}...")
                
                patterns = [
                    (r'####\s*\S+', "#### 格式"),
                    (r'\\boxed\{[^}]+\}', "\\boxed{} 格式"), 
                    (r'<answer>.*?</answer>', "<answer> 格式"),
                ]
                
                for pattern, desc in patterns:
                    match = re.search(pattern, text, re.IGNORECASE)
                    if match:
                        # print(f"   ✅ 找到 {desc}: {match.group()}")
                        return 1.0
                
                # 检查数字
                numbers = re.findall(r'\b\d+\b', text[-100:])
                if numbers:
                    # print(f"   ⚠️  找到数字但无标准格式: {numbers[-1]}")
                    return 0.5
            ...
            # 详细调试准确率奖励
            def debug_accuracy_reward(pred: str, truth: str) -> float:
                # print(f"🔍 检查准确率奖励:")
                # print(f"   预测: {pred[-100:]}...")  # 最后100字符
                # print(f"   答案: {truth}")
                
                # 从预测中提取数字
                pred_numbers = re.findall(r'####\s*(\d+)', pred)
                if not pred_numbers:
                    pred_numbers = re.findall(r'\\boxed\{(\d+)\}', pred)
                if not pred_numbers:
                    pred_numbers = re.findall(r'\b(\d+)\b', pred[-100:])
                
                # 从真实答案中提取数字  
                truth_numbers = re.findall(r'####\s*(\d+)', truth)
                if not truth_numbers:
                    truth_numbers = re.findall(r'\b(\d+)\b', truth)
                
                # print(f"   提取的预测数字: {pred_numbers}")
                # print(f"   提取的真实数字: {truth_numbers}")
                
                if pred_numbers and truth_numbers:
                    pred_final = pred_numbers[-1]
                    truth_final = truth_numbers[-1]
                    result = 1.0 if pred_final == truth_final else 0.0
                    # print(f"   对比: {pred_final} == {truth_final} -> {result}")
                    return result
                else:
                    # print("   ❌ 无法提取数字进行对比")
                    return 0.0
            ...
            # 计算奖励
            format_score = debug_format_reward(pred_answer)
            accuracy_score = debug_accuracy_reward(pred_answer, groud_truth)
            
            score = {
                "format": float(format_score),
                "accuracy": float(accuracy_score),
                "score": float(format_score + accuracy_score)
            }
